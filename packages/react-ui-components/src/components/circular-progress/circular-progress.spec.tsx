...
describe('CircularProgress', () => {
  ...
  it('can be sizes', () => {
    ...
    expect(container.firstChild).toHaveClass('rmwc-circular-progress--size-xsmall');

    rerender(<CircularProgress size="small" />);
    expect(container.firstChild).toHaveClass('rmwc-circular-progress--size-small');

    rerender(<CircularProgress size="medium" />);
    expect(container.firstChild).toHaveClass('rmwc-circular-progress--size-medium');

    rerender(<CircularProgress size="large" />);
    expect(container.firstChild).toHaveClass('rmwc-circular-progress--size-large');

    rerender(<CircularProgress size="xlarge" />);
    expect(container.firstChild).toHaveClass('rmwc-circular-progress--size-xlarge');
  ...
  it('can have a different max / min', () => {
    const { container, rerender } = render(<CircularProgress min={0} max={100} progress={30} />);

...
export default function Readme() {
  return (
    ...
    >
      ...
      <DocsP>Selects come in three different styles: standard,outlined, and enhanced.</DocsP>
      ...
      <DocsP>The Select component has the same behaviors as a native HTML select and be both controlled and uncontrolled.</DocsP>
      ...
      <DocsP>
        To fit common use cases, RMWC Select provides a data driven method for rendering select menus. There are multiple formats you can pass data
        in, use the one that best fits your requirements. To make your label not float by default and to have an unselected blank value, set the
      ...
      <DocsExample label="Value => Label Map">
        <Select label="Object map" options={{ '1': 'Cookies', '2': 'Pizza', '3': 'Icecream' }} />
      </DocsExample>

      <DocsExample label="Array">
        <Select label="Simple Array" placeholder="-- Select One --" options={['Cookies', 'Pizza', 'Icecream']} />
      </DocsExample>

      <DocsSubtitle>Manually Building the List</DocsSubtitle>
      <DocsP>If you want full control over the child `ListItems`, you can manually build the list yourself.</DocsP>
      ...
      <DocsP>Both native and enhanced Selects can contain option groups. Just nest additional options arrays in your data.</DocsP>
      ...
      <DocProps src={propsSrc} components={[{ displayName: 'Select', component: Select }]} />
    </Docs>
  );
}

export const galleryExample = <Select options={[]} placeholder="--Select One--" label="Favorite Food" />;

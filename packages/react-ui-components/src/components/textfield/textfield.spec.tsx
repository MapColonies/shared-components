...
describe('TextField', () => {
  it('renders', () => {
    const { asFragment } = render(<TextField label="test" placeholder="test" />);
    expect(screen.getByText('test')).toBeInTheDocument();
    expect(asFragment()).toMatchSnapshot();
  });

  it('can autoFocus', () => {
    const { container } = render(<TextField label="test" placeholder="test" autoFocus />);
  ...
  it('can have helpText', () => {
    ...
    expect(container.getElementsByClassName('mdc-text-field-helper-text--validation-msg')).toHaveLength(1);
    expect(container.getElementsByClassName('mdc-text-field-helper-text--persistent')).toHaveLength(1);
  });
  ...
  it('can be bound', () => {
    render(<TextField placeholder="test" value="hello world" onChange={() => {}} />);
    expect(screen.getByRole('textbox')).toHaveValue('hello world');
  });

  it('can be textarea', () => {
    const { asFragment } = render(<TextField placeholder="test" value="hello world" textarea onChange={() => {}} />);
    expect(screen.getByText('hello world')).toBeInTheDocument();
    expect(asFragment()).toMatchSnapshot();
  });

  it('can have custom classnames on input', () => {
    const { container } = render(<TextField className={'my-custom-classname'} />);
    expect(container.firstChild).toHaveClass('my-custom-classname');
  });
  ...
  it('can be required', async () => {
    const { container } = render(<TextField value="" onChange={() => {}} required />);
    const getValid = () => container.getElementsByClassName('mdc-text-field--invalid').length === 0;
  ...
  it('label floats on dynamic change', async () => {
    const { container } = render(<TextField label="test" value="" onChange={() => {}} />);
    expect(container.getElementsByClassName('mdc-floating-label--float-above')).toHaveLength(0);
    userEvent.type(screen.getByRole('textbox'), 'foo');
    await wait(100);
    expect(container.getElementsByClassName('mdc-floating-label--float-above')).toHaveLength(1);
  });
...
describe('TextFieldHelperText', () => {
  it('renders', () => {
    const { asFragment } = render(<TextFieldHelperText>Hello</TextFieldHelperText>);

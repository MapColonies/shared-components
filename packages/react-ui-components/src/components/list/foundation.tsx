...
export const useListFoundation = (props: ListProps & React.HTMLProps<any>) => {
  ...
  const { foundation, ...elements } = useFoundation({
    ...
    foundation: ({ rootEl, emit }) => {
      return new MDCListFoundation({
        ...
        getFocusedElementIndex: () => listElements(rootEl.ref).indexOf(document.activeElement as HTMLLIElement),
        listItemAtIndexHasClass: (index: number, className: string) => {
          const element = listElements(rootEl.ref)[index];
          return !!element?.classList.contains(className);
        },
        setAttributeForElementIndex: (index: number, attr: string, value: string | number) => {
        ...
        setTabIndexForListItemChildren: (listItemIndex: number, tabIndexValue: string | number) => {
          const element = listElements(rootEl.ref)[listItemIndex];
          const listItemChildren: Element[] = [].slice.call(element.querySelectorAll(MDCListFoundation.strings.CHILD_ELEMENTS_TO_TOGGLE_TABINDEX));
          listItemChildren.forEach((ele) => ele.setAttribute('tabindex', String(tabIndexValue)));
        },
        ...
        isCheckboxCheckedAtIndex: (index: number) => {
          const listItem = listElements(rootEl.ref)[index];
          const toggleEl = listItem.querySelector(MDCListFoundation.strings.CHECKBOX_SELECTOR) as HTMLInputElement | null;

          return toggleEl ? toggleEl.checked : false;
        },
        setCheckedCheckboxOrRadioAtIndex: (index: number, isChecked: boolean) => {
          const listItem = listElements(rootEl.ref)[index];
          const toggleEl = listItem.querySelector(MDCListFoundation.strings.CHECKBOX_RADIO_SELECTOR) as HTMLInputElement | null;
  ...
  const handleClick = useCallback(
    (evt: React.MouseEvent) => {
      ...
      const toggleCheckbox = !matches(evt.target as HTMLElement, MDCListFoundation.strings.CHECKBOX_RADIO_SELECTOR);
    ...
    (evt: React.KeyboardEvent<HTMLElement> & KeyboardEvent) => {
      ...
      if (index >= 0) {
        foundation.handleKeydown(
          evt,
          evt.target instanceof Element && evt.target.classList.contains(MDCListFoundation.cssClasses.LIST_ITEM_CLASS),
          index
        );
      }
    },

...
export default function Readme() {
  return (
    ...
    >
      ...
      <DocsP>
        You can compose a menu with the given components, and manually manage the open state. `Menu` expects MenuItems as children while `MenuSurface`
      ...
      <DocsP>RMWC provides a convenience `SimpleMenu` component that takes a handle as a prop, and manages the open state for you.</DocsP>
      ...
      <DocsExample>
        {function Example() {
          const [anchorCorner, setAnchorCorner] = React.useState<any>('topLeft');

          return (
            <>
              <MenuSurfaceAnchor>
                <MenuSurface anchorCorner={anchorCorner} open={true}>
                  <div style={{ padding: '1rem', width: '8rem' }}>anchorCorner: {anchorCorner}</div>
                </MenuSurface>
              ...
              <Select
                ...
                options={['topLeft', 'topRight', 'bottomLeft', 'bottomRight', 'topStart', 'topEnd', 'bottomStart', 'bottomEnd']}
              />
            </>
          );
        }}
      ...
      <DocsP>
        Occasionally, you may find your menu being cut off from being inside a container that is styled to be `overflow:hidden`. RMWC provides a
      ...
      <DocsP>Now you can use the `renderToPortal` prop. Below is a contrived example of a menu being cut off due to `overflow: hidden`.</DocsP>
            ...
            <>
              ...
              >
                <MenuSurfaceAnchor>
                  ...
                  <Menu open={menuIsOpen} onClose={() => setMenuIsOpen(false)} renderToPortal={renderToPortal}>
                    {options.map((o) => (
              ...
              <Checkbox checked={renderToPortal} onChange={(evt) => setRenderToPortal(evt.currentTarget.checked)} label="renderToPortal" />
            </>
          );
        }}
      </DocsExample>
...
export const galleryExample = (
  <div aria-hidden="false" className="mdc-menu  mdc-menu-surface--open mdc-menu-surface" style={{ position: 'static' }}>

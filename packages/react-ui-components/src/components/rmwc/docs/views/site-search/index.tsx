...
const searchGoogle = async (val: string, abortController: AbortController) => {
  const { items = [] } = await fetch(
    `https://www.googleapis.com/customsearch/v1/siterestrict?key=${import.meta.env.REACT_APP_CUSTOM_SEARCH_KEY}&cx=${
      import.meta.env.REACT_APP_CUSTOM_SEARCH_ID
    }&q=${val}`,
  ...
  return (
    items
      ...
      .map((r) => ({
        ...
        url: ((r.formattedUrl.split('?').pop() || '').split('&').find((p) => p.startsWith('p=')) || '').slice(2),
      }))
  );
};
  ...
  return (
    <MenuSurfaceAnchor className={styles.siteSearchWrapper}>
      <TextField
        ...
        onKeyDown={(evt) => {
          if (evt.which === 40) {
            const listItem = document.querySelector<HTMLAnchorElement>(`.${styles.siteSearchMenu} a`);
      ...
      <Menu
        ...
        onSelect={(evt) => {
          window.scrollTo(0, 0);
          navigate((evt.detail.item as HTMLAnchorElement).href.split('/').pop() || '/');
          document.activeElement && (document.activeElement as HTMLElement).blur();
        }}
      >
        <MenuItems twoLine>
          {results.map((r) => (
            <SimpleListItem tag={Link} to={r.url} graphic={r.icon} key={r.id} text={r.sectionName} secondaryText={r.snippet} />
          ))}
          ...
          {!abortControllerRef.current && !results.length && <SimpleListItem text="No results found" />}
        </MenuItems>
      </Menu>
    </MenuSurfaceAnchor>
  );

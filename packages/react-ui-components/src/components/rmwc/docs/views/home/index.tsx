...
function Example({ ex, name, url }: { ex: React.ReactNode; name: string; url: string }) {
  return (
    <Link to={url} className={styles.example} onClick={() => window.scrollTo(0, 0)}>
    ...
    <React.Fragment>
      <header className={styles.header}>
        <div className="container">
          ...
          <Grid className={styles.featureGrid}>
            ...
            <GridCell span={4}>
              <Card theme="background" tag="a" href="https://material.io/components/web/">
                <div style={{ padding: '1rem' }}>
                  <Typography use="headline6" tag="div">
                    Material Accuracy
                  </Typography>
            ...
            <GridCell span={4}>
              <Link to="/styling-theming">
                <Card style={{ backgroundColor: '#212121' }} theme="textPrimaryOnDark">
                  <div style={{ padding: '1rem' }}>
                    <Typography use="headline6" tag="div">
                      Easy to Customize
                    </Typography>
      ...
      <div className={styles.examples}>
        {galleryContent.map((ex) => {
          return <Example key={ex.label} name={ex.label} ex={ex.gallery} url={ex.url!} />;
        })}
      </div>
    </React.Fragment>
  );

  ...
  render() {
    ...
    return (
      <MenuSurfaceAnchor>
          ...
          {Object.keys(THEMES).map((themeName) => {
            ...
            return (
              ...
              >
                ...
                />
                <ListItemGraphic icon={themeName === selectedThemeName ? 'check' : ''} />
                {themeName}
                <ListItemMeta>
          ...
          >
            {this.state.activeTabIndex === 0 ? (
              <div>
                ...
                <span className="token keyword">import</span> {'{'} ThemeProvider {'}'} <span className="token keyword">from</span>{' '}
                ...
                {Object.entries(selectedTheme).map(([t, val]: [string, any], index, arr) => (
                  <div
                    key={t}
                    style={{
                      display: 'flex',
              ...
              <div>
                ...
                {Object.entries(selectedTheme).map(([t, val]: [string, any]) => (
                  <div
                    key={t}
                    style={{
                      display: 'flex',
        ...
        <TopAppBarActionItem onClick={() => this.setState({ open: !this.state.open })} theme="onPrimary" icon="color_lens" />
      </MenuSurfaceAnchor>
    );
  }
}
...
const ColorBlock = ({ color, size = 1.5 }: { color: string; size?: number }) => (
  <div
    style={{

...
import { Drawer, DrawerContent, DrawerAppContent, DrawerProps } from '@rmwc/drawer';
...
const MainMenuItem = ({ url, label }: { url?: string; label: string }) => {
  ...
  return (
    <ListItem tag={Link} to={url} onClick={() => window.scrollTo(0, 0)} activated={location.pathname === url}>
      <span>{label}</span>
    </ListItem>
  );
};
...
function AppBar({ onNavClick, children }: { onNavClick: (evt: React.SyntheticEvent<HTMLElement>) => void; children: React.ReactNode }) {
  return (
    <>
      <TopAppBar fixed className="app__top-app-bar">
        <TopAppBarRow>
          ...
          <TopAppBarSection alignEnd>
            {children}
            <TopAppBarActionItem tag="a" href="https://github.com/jamesmfriedman/rmwc" icon={<GithubSvg />} />
          </TopAppBarSection>
        </TopAppBarRow>
      </TopAppBar>
      <TopAppBarFixedAdjust />
...
export function App() {
  ...
  const [pageId, setPageId] = useState(`page-${location.pathname.split('/').pop() || 'home'}`);
  const [theme, setTheme] = useState(window.localStorage.getItem('rmwcTheme') || 'Baseline');
  ...
  return (
    <ThemeProvider options={getTheme(theme)} className="app__root" tag="div" id={pageId}>
      <PortalProvider>
        <AppBar
          onNavClick={(evt) => {
        ...
        <div className="demo-content">
          <Nav open={menuIsOpen} dismissible={!isMobile} modal={isMobile} onClose={() => setMenuIsOpen(false)} />
      ...
      {options.map((m) => {
        if (m.options) {
          return (
            <CollapsibleList
              key={m.label}
              defaultOpen={m.label === 'Components' || m.options?.some((o) => o.url && location.pathname === o.url)}
              handle={<SimpleListItem text={m.label} metaIcon="chevron_right" />}
            >
...
function getDocRoutes({ options }: { options: MenuItemT[] }): React.ReactElement[] {
  return options.flatMap((value) => {
    ...
    return [<Route path={value.url!} key={value.label} element={<Component />} />];
  });
}

...
export const useTabScrollerFoundation = (props: TabScrollerProps & React.HTMLProps<any>) => {
  const { foundation, ...elements } = useFoundation({
    ...
    api: ({ foundation, contentEl }): TabScrollerApi => {
      ...
      return {
        ...
        incrementScroll: (scrollXIncrement: number) => f.incrementScroll(scrollXIncrement),
    ...
    foundation: ({ rootEl, areaEl, contentEl }) => {
      return new MDCTabScrollerFoundation({
        eventTargetMatchesSelector: (evtTarget: EventTarget, selector: string) => matches(evtTarget as HTMLElement, selector),
        addClass: (className: string) => rootEl.addClass(className),
        removeClass: (className: string) => rootEl.removeClass(className),
        addScrollAreaClass: (className: string) => areaEl.addClass(className),
        setScrollAreaStyleProperty: (prop: string, value: string) => areaEl.setStyle(prop, value),
        setScrollContentStyleProperty: (prop: string, value: string) => contentEl.setStyle(prop, value),
        getScrollContentStyleValue: (propName: string) => {
          const val = contentEl.ref && window.getComputedStyle(contentEl.ref).getPropertyValue(propName);

          return val || 'none';
        },
        setScrollAreaScrollLeft: (scrollX: number) => areaEl.ref && (areaEl.ref.scrollLeft = scrollX),
        getScrollAreaScrollLeft: () => (areaEl.ref ? areaEl.ref.scrollLeft : 0),
        getScrollContentOffsetWidth: () => (contentEl.ref ? contentEl.ref.offsetWidth : 0),
        getScrollAreaOffsetWidth: () => (areaEl.ref ? areaEl.ref.offsetWidth : 0),
        computeScrollAreaClientRect: () => (areaEl.ref ? areaEl.ref.getBoundingClientRect() : emptyClientRect),
        computeScrollContentClientRect: () => (contentEl.ref ? contentEl.ref.getBoundingClientRect() : emptyClientRect),
        computeHorizontalScrollbarHeight: () => scrollerUtil.computeHorizontalScrollbarHeight(document),
      });
    },
  });
  ...
  const handleTransitionEnd = (evt: React.TransitionEvent | TransitionEvent) => {

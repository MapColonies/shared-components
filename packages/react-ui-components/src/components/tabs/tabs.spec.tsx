...
describe('Tabs', () => {
  ...
  it('sets initial active tab', async () => {
    ...
    await waitFor(() => expect(screen.getByText('2').parentElement?.parentElement).toHaveClass('mdc-tab--active'));
  });
  ...
  it('can be used as a controlled element', async () => {
    ...
    await waitFor(() => expect(container.getElementsByClassName('mdc-tab--active')).toHaveLength(1));
  });

    ...
    return (
      <React.Fragment>
        ...
        <ol>
          ...
          <li>Observe that `preventOutsideDismiss` is `false` and is working as expected</li>
          <li>Click the "TOGGLE" button</li>
          <li>Observe that the dialog is CORRECTLY NOT DISMISSED when clicking outside</li>
          <li>Observe that the dialog is INCORRECTLY DISMISSED when using the escape key</li>
        </ol>
        <br />
        <Button raised icon="bug_report" onClick={() => setOpen(true)}>
          Open Dialog
        </Button>
        <Dialog open={open} preventOutsideDismiss={preventOutsideDismiss} onClosed={() => setOpen(false)}>
          <DialogTitle>Prevent Dismiss Demo</DialogTitle>
          <DialogContent>Prevent outside dismiss is currently: {preventOutsideDismiss.toString()}</DialogContent>
          <Button onClick={() => setPreventOutsideDismiss((p) => !p)}>Toggle</Button>
        </Dialog>
      </React.Fragment>
    );
  })

...
export default function Readme() {
  return (
    <Docs
      ...
      styles={['@material/data-table/dist/mdc.data-table.css', '@rmwc/data-table/data-table.css', '@rmwc/icon/icon.css']}
      examples={examples}
    >
      <DocsSubtitle>Standard Table</DocsSubtitle>

      <DocsP>
        The DataTable components are intended to be flexible, properly styled, Material compliant HTML tables. Because of the complexities of working
        with datasets (especially large ones), the DataTable component DOES NOT handle pagination, data fetching, sorting, or performance of long
        lists.
      </DocsP>
      ...
      <DocsP>
        You can set a fixed sized for your table container to make it scrollable. Additionally, you can specify `stickyRows` or `stickyColumns` to
        affix rows or columns. Currently, only 1 row / column is supported but more may be supported in a future release.
      </DocsP>
      ...
      <DocsExample>
        {function Example() {
          ...
          return (
            <>
              <DataTable style={{ height: '300px', width: '375px' }} stickyRows={rows} stickyColumns={cols}>
                <DataTableContent>
                  <DataTableHead>
                    <DataTableRow>
              ...
              <div className="doc-controls">
                <Select label="Sticky Rows" options={['0', '1']} value={String(rows)} onChange={(evt) => setRows(Number(evt.currentTarget.value))} />
                <Select label="Sticky Cols" options={['0', '1']} value={String(cols)} onChange={(evt) => setCols(Number(evt.currentTarget.value))} />
              </div>
            </>
          );
        }}
      ...
      <DocsP>DataTables play nice with the rest of the RMWC form controls. You are responsible for scripting your own selection behavior.</DocsP>
      ...
      <DocsP>If you just need to throw a table on the screen, you can pass an array of data to SimpleDataTable.</DocsP>

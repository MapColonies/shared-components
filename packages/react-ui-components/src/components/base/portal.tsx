import React, { useEffect, useState, useContext, useCallback, MutableRefObject } from 'react';
...
export function PortalChild({
  ...
  if (renderTo) {
    if (portalEl) {
      return <div ref={menuSurfaceDomPositionRef}>{createPortal(children, portalEl)}</div>;
    } else {
      return null;
    }
  }

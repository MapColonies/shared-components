...
it('sets the correct values for the table cell properties', () => {
  ...
  const firstTableCellProps = wrapper.findWhere((n) => n.type() === TableCell && n.key() === headCells[0].id).props();
  /* eslint-disable @typescript-eslint/no-unsafe-assignment */
  const secondTableCellProps = wrapper.findWhere((n) => n.type() === TableCell && n.key() === headCells[1].id).props();
...
it('sets the correct values for the sort label properties', () => {
  ...
  const firstSortLabelProps = wrapper.findWhere((n) => n.type() === TableSortLabel && n.prop('children') === headCells[0].label).props();
  const secondSortLabelProps = wrapper.findWhere((n) => n.type() === TableSortLabel && n.prop('children') === headCells[1].label).props();
...
it('calls onRequestSort with the correct params', () => {
  ...
  const wrapper = shallow(
    <SmartEnhancedTableHead headCells={headCells} isCollapseable={false} onRequestSort={handleRequestSort} order="desc" orderBy={'first'} />
  );

  wrapper.findWhere((n) => n.type() === TableSortLabel && n.prop('children') === headCells[0].label).simulate('click', {});
